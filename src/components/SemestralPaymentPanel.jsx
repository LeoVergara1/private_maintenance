...
export default function SemestralPaymentPanel({ onPaymentCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Error al registrar pago semestral:', err);
  ...
  const getSemesterMonths = (semester) => {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            ...
            Registrar Pago Semestral
          </h2>
          <p className="text-sm text-gray-600">Registra un pago para 6 meses</p>
        </div>
        <button
        ...
        <form onSubmit={handleSubmit} className="bg-gray-50 rounded-lg p-6 space-y-4 mb-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-3">
              Selecciona los Meses (6 meses para semestral)
            </label>
            <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
              {Array.from({ length: 12 }, (_, i) => i + 1).map(month => (
                <label key={month} className="flex items-center gap-2 p-3 border border-gray-300 rounded-lg hover:bg-gray-100 cursor-pointer">
                  <input
                    type="checkbox"
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Agregar comentarios sobre el pago semestral..."
            />
          </div>
          ...
          <div className="flex gap-3">
            ...
            >
              {loading ? 'Registrando...' : 'Registrar Pago Semestral'}
            </button>
            <button
              type="button"
